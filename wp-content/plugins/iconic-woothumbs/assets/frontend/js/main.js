(function($, document) {
    ...
    var iconic_woothumbs = {
        ...
        hide_controls: function( product_object ) {
            var $controls = product_object.all_images_wrap.find('.slick-dots, .iconic-woothumbs-fullscreen, .iconic-woothumbs-images__arrow');
        ...
        setup_images_events: function( product_object ) {
            ...
            product_object.images.on('init', function( event, slick ) {
                ...
                var $current_slide = product_object.images.find('.iconic-woothumbs-images__slide.slick-active'),
        ...
        replace_images: function( product_object, images, callback ) {
            ...
            temp_images.container.iconicImagesLoaded().done( function() {
        ...
        create_temporary_images: function( images, product_object ) {
            ...
            $.each( images, function( index, image_data ){
                ...
                if( image_count > 1 && iconic_woothumbs.thumbnails_enabled() ) {
	                ...
	                var play_icon = image_data.media_embed ? iconic_woothumbs.tpl.thumbnail_play_icon : '';
        ...
        reset_images: function( product_object ){
            ...
            if( product_object.all_images_wrap.hasClass( iconic_woothumbs.vars.reset_class ) ||product_object.all_images_wrap.hasClass( iconic_woothumbs.vars.loading_class ) ) { return; }
            ...
            var options = {
                ...
                getThumbBoundsFn: function( index ) {
					var pageYScroll = window.pageYOffset || document.documentElement.scrollTop,
						rect = product_object.images.find('.iconic-woothumbs-images__slide.slick-active img')[0].getBoundingClientRect();
        ...
        stop_photoswipe_media: function() {
	        var $media = $('.iconic-woothumbs-fullscreen-video-wrapper iframe');
        ...
        stop_media: function( product_object ) {
            ...
            var $media = product_object.images.find( 'iframe' );
            ...
            $media.each(function( index, media ){
	            var $parent = $( media ).closest( '.iconic-woothumbs-images__slide' );
		...
		base.showViewer = function(e){
			...
			if(base.isBigImageReady ==-1)
			{
				...
				fastImg(base.options.bigImageSrc, function () {
					...
					{
						base.$viewer.append('<img src="'+base.$el.attr('src')+'" class="zm-fast" style="position:absolute;width:'+this.width+'px;height:'+this.height+'px" nopin="nopin" \>');
						base.isBigImageReady = 1;
...
jQuery(function($){
    ...
    $('li.tooltipstered').on('click',function(){
        set_iconic_image_height();
    });

    $(window).resize(function(){
        var resize_width = $(window).width();
        if (resize_width != win_width){
            win_width = resize_width;
            set_iconic_image_height();
        }
    });

    function set_iconic_image_height () {
        var iconic_image_height = parseInt($('.iconic-woothumbs-all-images-wrap').css('height'));
        var iconic_woothumbs_height = parseInt($('.iconic-woothumbs-images-wrap').css('width'));
        var iconic_thumbnail_height = parseInt($('.iconic-woothumbs-thumbnails-wrap').css('height'));
        var iconic_total_height = iconic_woothumbs_height+iconic_thumbnail_height;

        if ((iconic_image_height <= iconic_total_height) ||(iconic_image_height >= iconic_total_height+20)){
            $('.iconic-woothumbs-all-images-wrap').css('min-height',iconic_total_height);
            $('.iconic-woothumbs-all-images-wrap').css('max-height',iconic_total_height);
        }       
    }
});
