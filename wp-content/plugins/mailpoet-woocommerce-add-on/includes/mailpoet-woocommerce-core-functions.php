...
function is_user_subscribed($user_id, $email) {
	$subscriber_data = '';
	$model_user = WYSIJA::get('user','model');

	if (!empty($email)) {
		$subscriber_data = $model_user->getOne(false, array('email' => trim($email)));
	}

	if (empty($subscriber_data) || !isset($subscriber_data['user_id'])) {
		$subscriber_data = (array)$model_user->getOne(false, array( 'wpuser_id' => $user_id));

		if (empty($subscriber_data) || !isset($subscriber_data['user_id'])) {
			$user = get_userdata($user_id);
			$subscriber_data = $model_user->getOne(false, array('email' => trim($user->user_email)));
	...
	return $subscriber_data['status'] != -1;
}
...
function on_process_order(){
	...
	if( isset( $double_optin ) && $double_optin == 'yes' ){
		...
		$subscriber_id = !empty($subscriber_id) && isset($subscriber_id['user_id']) ? $subscriber_id['user_id'] : $subscriber_id;
		...
		$user_model->update( array( 'status' => -1 ), array( 'wpuser_id' => get_current_user_id() ) ); // This is to make sure the customer is not subscribed already.
